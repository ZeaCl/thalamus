defmodule ThalamusWeb.API.LoginController do
  ...
  # Internal client for API login (not exposed publicly)
  # This client is used internally to generate tokens for direct login
  @internal_client_id "thalamus_api_login"
  ...
  defp get_or_create_internal_client do
    # Try to find the internal client
    case PostgreSQLOAuth2ClientRepository.find_by_client_id(@internal_client_id) do
      ...
      {:error, :not_found} ->
        ...
        # This should normally be done via seeds, but we'll create it here as fallback
  ...
  defp create_internal_client do
    ...
    # Use scopes as strings (repository stores them as strings, not Scope value objects)
    scopes = get_default_user_scopes()
    ...
    {:ok, client} =
      OAuth2Client.new(%{
        ...
        # client_secret will be auto-generated by OAuth2Client.new()
    ...
    # Save to database with the specific client_id string
    case PostgreSQLOAuth2ClientRepository.save(client) do
      {:ok, saved_client} -> saved_client
      {:error, _} -> client  # Return unsaved client as fallback
    end
  end

  defp get_or_create_system_organization do
    # For now, we'll generate a new UUID
    # In production, this should be a fixed system organization
