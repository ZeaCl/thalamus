...
describe('secret', () => {
  beforeEach(async () => { output = ''; server = createTestServer(); fs.mkdirSync(path.dirname(CFG), { recursive: true }); mock.method(process, 'exit', (c) => { if (c !== 0) throw new Error(`exit(${c})`); }); mock.method(console, 'log', (...a) => { output += a.join(' ') + '\n'; }); mock.method(console, 'error', (...a) => { output += a.join(' ') + '\n'; }); });
  ...
  it('list success', async () => { const API = await server.start(); server.get('/api/secrets', 200, { data: [{ id: 's1', name: 'k', provider: 'openai' }] }); const p = await S(API); await h(p, 'secret list')(); assert.match(output, /openai/); assert.match(output, /s1/); });
  ...
  it('resolve success', async () => { const API = await server.start(); server.get('/oauth/userinfo', 200, { sub: 'u1' }); server.any(/\/api\/internal\/secrets\/resolve/, 200, { value: 'sk-123' }); const p = await S(API); await h(p, 'secret resolve')({ provider: 'openai' }); assert.match(output, /sk-123/); });
  it('resolve not found', async () => { const API = await server.start(); server.get('/oauth/userinfo', 200, { sub: 'u1' }); server.any(/\/api\/internal\/secrets\/resolve/, 404, { error: 'not found' }); const p = await S(API); try { await h(p, 'secret resolve')({ provider: 'x' }); } catch(e) {} assert.match(output, /not found|Error/); });
});
