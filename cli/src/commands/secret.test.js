...
describe('secret', () => {
  ...
  it('list missing id', async () => { const API = await server.start(); server.get('/api/secrets', 200, { data: [{ name: 'no-id', provider: 'aws' }] }); const p = await S(API); await h(p, 'secret list')(); assert.match(output, /\(no id\)/); });
