defmodule ThalamusWeb.OAuth2.AuthorizationControllerTest do
  ...
  describe "POST /oauth/authorize - consent processing" do
    test "redirects with authorization code when user approves", %{conn: conn, user: user, client: client} do
      conn = conn
    ...
    test "includes PKCE parameters in authorization code", %{conn: conn, user: user, client: client} do
    ...
    test "returns error when user not authenticated", %{conn: conn, client: client} do
      conn = post(conn, ~p"/oauth/authorize", %{
        decision: "approve",
        client_id: to_string(client.id),
        redirect_uri: "http://localhost:3000/callback",
        scope: "read"
  ...
  describe "authorization code expiration" do
    test "authorization code expires after configured time", %{conn: conn, user: user, client: client} do
  ...
  describe "scope validation" do
    test "allows requested scopes that are subset of client allowed scopes", %{conn: conn, user: user, client: client} do
      ...
      |> post(~p"/oauth/authorize", %{
        ...
        client_id: to_string(client.id),
        redirect_uri: "http://localhost:3000/callback",
        scope: "read",  # Subset of allowed scopes
        state: "state_123"
      })
  ...
  describe "redirect URI validation" do
    ...
    test "rejects redirect URI not in client allowed list", %{conn: conn, user: user, client: client} do
      conn = conn
      |> put_session(:user_id, to_string(user.id))
      |> post(~p"/oauth/authorize", %{
        decision: "approve",
