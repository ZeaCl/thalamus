defmodule ThalamusWeb.API.AgentTokenControllerTest do
  ...
  describe "POST /api/internal/agent-token" do
    test "creates an agent token successfully when valid user_id is provided", %{conn: conn} do
      ...
      {:ok, saved_org} = Thalamus.Infrastructure.Repositories.PostgreSQLOrganizationRepository.save(org)
      saved_org_uuid = String.replace_prefix(Thalamus.Domain.ValueObjects.OrganizationId.to_string(saved_org.id), "org_", "")
      ...
      saved_user_uuid = String.replace_prefix(Thalamus.Domain.ValueObjects.UserId.to_string(saved_user.id), "user_", "")
      
      conn = post(conn, ~p"/api/internal/agent-token", %{
