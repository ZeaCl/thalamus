defmodule ThalamusWeb.API.OAuth2ClientControllerTest do
  ...
  describe "POST /api/clients" do
    ...
    test "creates client with default grant types and scopes", %{conn: conn, org: org, access_token: token} do
      conn = conn
      |> put_req_header("authorization", "Bearer #{token}")
      |> post(~p"/api/clients", %{
        name: "Default Client",
  ...
  describe "PATCH /api/clients/:id" do
    ...
    test "requires authentication", %{conn: conn, org: org} do
      ...
      conn = patch(conn, ~p"/api/clients/#{client.id}", %{
        name: "New Name"
      })
