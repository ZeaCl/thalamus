defmodule Thalamus.Infrastructure.Repositories.PostgreSQLOAuth2ClientRepositoryTest do
  ...
  describe "save/1" do
    ...
    test "saves client with scopes" do
      ...
      assert "profile" in Enum.map(saved_client.allowed_scopes, &to_string/1)
      assert "email" in Enum.map(saved_client.allowed_scopes, &to_string/1)
    end
    ...
    test "updates client with new scopes" do
      ...
      assert "profile" in Enum.map(result.allowed_scopes, &to_string/1)
    end
  ...
  describe "find_by_client_id/1" do
    ...
    test "finds manually-inserted client where DB id differs from client_id_string UUID" do
      # Simulates the bug from ZeaCl/zea#85: a client inserted manually
      # with client_id_string = "client_<uuid>" but id autogenerated differently
      org_uuid = create_organization()
      client_uuid = Ecto.UUID.generate()
      db_id = Ecto.UUID.generate()

      # Insert raw schema directly — id ≠ client_uuid (simulates manual insert)
      schema = %OAuth2ClientSchema{
        id: db_id,
        client_id_string: "client_#{client_uuid}",
        name: "Südlich Capital (manual insert)",
        client_type: :public,
      ...
      # The bug: find_by_client_id removed "client_" → looked for id=client_uuid
      # but actual DB id was db_id (different). Now fixed with 3rd condition.
      assert {:ok, found} =
               PostgreSQLOAuth2ClientRepository.find_by_client_id("client_#{client_uuid}")

      assert ClientId.to_string(found.id) == "client_#{db_id}"
      assert found.name == "Südlich Capital (manual insert)"
    end
  end
  ...
  describe "scope conversions" do
    test "converts standard OIDC scopes correctly" do
      ...
      assert "openid" in Enum.map(found_client.allowed_scopes, &to_string/1)
      assert "profile" in Enum.map(found_client.allowed_scopes, &to_string/1)
      assert "email" in Enum.map(found_client.allowed_scopes, &to_string/1)
    end

    test "converts custom scopes correctly" do
      {:ok, custom1} = Scope.new("api:read")
      {:ok, custom2} = Scope.new("api:write")
      ...
      assert length(found_client.allowed_scopes) == 2
      assert "api:read" in Enum.map(found_client.allowed_scopes, &to_string/1)
      assert "api:write" in Enum.map(found_client.allowed_scopes, &to_string/1)
    end
