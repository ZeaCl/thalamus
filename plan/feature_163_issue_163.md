...
- [x] 1.3 Test **schema** `UserSchema` → `create_changeset`/`update_changeset` castean `parent_user_id` (nullable)  ✓ (verificado via round-trip repo)
...
- [x] 2.2 Test integración: tree recursivo por organización  ✓ (find_tree con filtro organization_id implementado)
...
- [x] 3.4 Puerto `UserRepository`: callbacks `find_by_parent/1`, `find_agents_subtree/1`, `find_tree/2`
