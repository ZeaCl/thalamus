...
- [x] Correr `mix test` baseline para confirmar suite verde antes de tocar código  (1893 tests / 0 failures)

### Fase 1 — Tests unitarios (TDD, primero en fallar)

- [ ] 1.1 Test **domain entity** `User` → usa/carrea `parent_user_id` en `new/1`, `register/2`, `register_agent/4`
- [ ] 1.2 Test **domain entity** `User` → `agent_config` expone `role` para armar `reports`
- [ ] 1.3 Test **schema** `UserSchema` → `create_changeset`/`update_changeset` castean `parent_user_id` (nullable)
- [ ] 1.4 Test **repo (unit)** `find_by_parent/1` → devuelve hijos directos
- [ ] 1.5 Test **repo (unit)** tree recursivo → sub-árbol multi-nivel (capa a capa)
- [ ] 1.6 Test **repo (unit)** `find_agents_subtree/1` → solo agentes subordinados

### Fase 2 — Tests de integración (fallando primero)

- [ ] 2.1 Test integración: `parent_user_id` persistido y leído correctamente via repo (round-trip)
- [ ] 2.2 Test integración: tree recursivo por organización
- [ ] 2.3 Test controller: `/oauth/userinfo` incluye `reports` (agentes/hijos) cuando existen
- [ ] 2.4 Test controller: `/oauth/userinfo` omite/`[]` `reports` cuando no hay dependientes

### Fase 3 — Implementación

- [ ] 3.1 Migración `add_parent_user_id_to_users` (FK a `users.id`, nullable) + índices
- [ ] 3.2 `UserSchema`: agregar `parent_user_id` field + relación `belongs_to :parent`
- [ ] 3.3 `User` entidad: agregar `parent_user_id` a tipo/struct/`new/1`/constructores
- [ ] 3.4 Puerto `UserRepository`: callbacks `find_by_parent/1`, `find_agents_subtree/1`
- [ ] 3.5 Impl `PostgreSQLUserRepository`: `find_by_parent`, tree recursivo-eficiente, `find_agents_subtree`
- [ ] 3.6 Mapeo `schema_to_entity`/`entity_to_schema` con `parent_user_id` (prefijo `user_`)
- [ ] 3.7 `UserinfoController`: agregar `reports` al JSON
- [ ] 3.8 API REST: aceptar `parent_user_id` en create/update de usuario
